...
public class InformationCmd implements CommandAdapter {
    ...
    public void run(@NotNull CommandContext ctx) {
        final String p = MoonlightBot.get("prefix");
        ...
        MessageEmbed embed = new EmbedBuilder().setColor(MoonlightBot.NORMAL).setAuthor("Wszystko o " + ctx.getJDA().getSelfUser().getName(), null, ctx.getJDA().getSelfUser().getEffectiveAvatarUrl()).setDescription("> Dzień dobry! Jestem tu by dodać trochę funkcji i uczynić serwer ciekawszym. szymon9932 napisał mój kod w języku **Java**.\n• Wpisz `" + p + "pomoc`, by zobaczyć listę komend.\n• Użyj `" + p + "modules`, by otrzymać listę modułów.\n\n• **Zużycie pamięci:** " + (int) memory + " MB\n• **Start o czasie:** " + data + "\n\n> 📚 **Biblioteki**\n• [JDA](https://github.com/DV8FromTheWorld/JDA) \n• [LavaPlayer](https://github.com/sedmelluq/lavaplayer)").build();
